...
<div>
    {{-- HEADER --}}
    <x-header title="KYC - {{ $customer->public_name ?? $customer->user_name }}" separator progress-indicator>
        ...
        <x-slot:actions>
            <x-button label="Retour" icon="o-arrow-left" link="/customers/{{ $customer->customer_id }}" class="btn-outline" />
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div class="lg:col-span-2">
            ...
            <x-card title="Aucune donnée KYC" icon="o-exclamation-triangle" class="mt-6">
                <div class="py-8 text-center">
                    ...
                    <p class="mb-4 text-gray-600">Ce client n'a pas encore complété son processus KYC.</p>
                    <x-button label="Créer KYC" icon="o-plus" class="btn-primary" />
                </div>
            </x-card>
            @endif
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between p-3 rounded bg-base-200">
                        <div class="flex items-center space-x-3">
                            ...
                            <div>
                                <div class="font-medium">Niveau de confiance: {{ $customer->trust_level }}</div>
                                <div class="text-sm text-gray-500">Niveau de vérification actuel</div>
                            </div>
                        </div>
                        <span class="text-sm text-gray-500">
                            {{ $customer->create_time ? \Carbon\Carbon::parse($customer->create_time)->format('d/m/Y') : '-' }}
                        </span>
                    </div>
                    ...
                    @if($customer->status_change_time)
                    <div class="flex items-center justify-between p-3 rounded bg-base-200">
                        <div class="flex items-center space-x-3">
                            <x-icon name="o-arrow-path" class="w-5 h-5 text-info" />
                            <div>
                                <div class="font-medium">Changement de statut</div>
                                <div class="text-sm text-gray-500">{{ $customer->status_change_reason ?? 'Raison non spécifiée' }}</div>
                            </div>
                        </div>
                        <span class="text-sm text-gray-500">
                            {{ \Carbon\Carbon::parse($customer->status_change_time)->format('d/m/Y') }}
                        </span>
                    </div>
                    @endif
                </div>
        ...
        <div class="space-y-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            <span class="text-sm">Niveau de confiance</span>
                            <x-badge value="{{ $customer->trust_level }}" class="badge-info" />
                        </div>
                        <div class="flex items-center justify-between">
                            <span class="text-sm">Statut client</span>
                            <x-badge value="{{ $customer->status }}"
                                     class="badge-{{ $customer->status === 'ACTIVE' ? 'success' : 'neutral' }}" />
                        </div>
                    </div>
                </div>
            </x-card>
            ...
            <x-card title="Navigation" icon="o-map">
                <div class="space-y-2">
                    <x-button label="Détails Client" icon="o-user" link="/customers/{{ $customer->customer_id }}" class="w-full btn-outline" />
                    <x-button label="Transactions" icon="o-credit-card" link="/customers/{{ $customer->customer_id }}/transactions" class="w-full btn-outline" />
                    <x-button label="Retour à la liste" icon="o-list-bullet" link="/customers" class="w-full btn-ghost" />
                </div>
            </x-card>
        </div>
    </div>
